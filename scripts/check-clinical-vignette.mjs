...
const REGRESSION_KNOWLEDGE_STEMS = [
  ...
  // 2026-08-18 운영 실측(#230). 환자 도입도 있고 임상 정보도 있지만 발문이 지식형이다 —
  // 껍데기 증례의 전형. 아래 증례 표본을 통과시키려고 규칙을 풀 때 이것까지 새면 안 된다.
  '65세 남자가 건강검진에서 흉부 X선 검사상 종격동 확장(widened mediastinal shadow)이 ' +
    '발견되어 병원에 왔다. 환자는 마르판 증후군(Marfan syndrome)을 앓고 있으며 흡연력, ' +
    '고혈압 병력이 있다. 흉부 대동맥류에 대한 설명으로 옳은 것은?',
];
...
const GOOD_VIGNETTES = [
  ...
  // ── 2026-08-18 운영 실측(#230)에서 **지식형으로 잘못 세어졌던** 실제 문항 2개.
  //    둘 다 명백한 증례인데 판정 규칙이 조사·시제·검사명 표기를 못 넘었다:
  //      · "진찰 시"  — 규칙은 "진찰에서/진찰상"만 봤다
  //      · "CT 결과"  — 규칙은 "검사 결과"만 봤다
  //      · "혈압은 170/100" — 규칙은 조사 없는 "혈압 170"만 봤다
  //    그 결과 임상형 수확률이 3/10 으로 잡혔지만 실제로는 5/10 이었다.
  //    계측이 틀리면 프롬프트를 고칠 이유도 근거도 사라진다.
  '72세 남자가 갑작스러운 등 통증으로 응급실에 왔다. 통증은 \'찢어지는 듯하다\'고 표현하며 ' +
    '식은땀을 동반하였다. 과거력상 고혈압이 있었고, 규칙적인 약 복용은 하지 않았다고 한다. ' +
    '진찰 시 양측 요골 동맥 맥박이 촉지되지 않았다. 응급 흉부 CT 결과 상행 대동맥을 침범한 ' +
    '대동맥 박리가 확인되었다. 이 환자에게 가장 먼저 시행할 처치로 적절한 것은?',
  '65세 남자가 갑자기 발생한 심한 등 통증으로 응급실에 왔다. 통증은 찢어지는 듯한 양상이며 ' +
    '땀을 많이 흘리고 있었다. 혈압은 170/100 mmHg로 측정되었고, 양쪽 상지의 맥박 차이가 ' +
    '관찰되었다. 이 환자에게 절대 금기되는 약물은?',
];

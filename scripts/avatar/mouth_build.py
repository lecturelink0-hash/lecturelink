...
class LipsBuilder:
    def __init__(self, g, head_mesh, skin_mat='Skin', opts=None):
        ...
        self.o = dict(
            ...
            width_frac=0.64,   # 입 반폭 = 얼굴 반폭 × 이 값 (0.50 → 0.58 → 0.64: "입 크기 조금만 더")
            h_up=0.54,         # 윗입술 높이(입선→상단 경계, 중앙 기준) cm
            h_low=0.68,        # 아랫입술 높이 cm
            d_up=0.17,         # 윗입술 최대 두께(표면에서 앞으로) cm — 얇게(2026-08-26 피드백)
            d_low=0.22,        # 아랫입술 최대 두께 cm
            line_d=0.06,       # 입선에서 두 입술이 만나는 높이 cm
            corner_dip=-0.15,  # 입꼬리가 입선 중앙보다 내려가는 양 cm — 음수면 올라감(살짝 미소; 일직선은 "중성마녀 입" 피드백)

...
## 2026-08-16 베타 제공 계정 구독 관리 진입 보완

- 요청 및 목적: 실제 운영 계정이 유료 티어를 사용 중이지만 subscriptions 레코드가 없는 베타 제공 상태여도 구독 관리 화면에 진입할 수 있게 보완.
- 변경 파일:
  - `app/(app)/mypage/page.tsx`: free가 아닌 플랜은 `구독 관리`로 진입하도록 조건 수정.
  - `app/(app)/subscription/page.tsx`: 결제 레코드가 없는 베타 제공 계정도 현재 플랜을 확인할 수 있게 처리. 결제일·카드 정보·해지 버튼은 실제 데이터가 없으므로 표시하거나 동작시키지 않음.
- 유지한 요소: 활성 정기결제 계정의 기존 요금제 변경, 해지 사유, 확인 모달, 해지 API는 변경하지 않음.
- 검사 결과: 두 변경 파일 TypeScript 구문 검사, Impeccable 레이아웃 검사, `git diff --check` 통과.
- 사용자 최종 확인 결과: 운영 배포 후 확인 대기.
- 최종 상태: 조건부 완료 — main 반영 전.

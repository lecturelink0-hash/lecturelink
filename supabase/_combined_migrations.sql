-- 렉처링크 통합 마이그레이션 (00001~00021 + 00036, pg_cron(00011) 제외)
-- 새 Supabase 프로젝트 SQL Editor 에 전체 붙여넣고 Run
--
-- ⚠️ 이 파일은 낡았다 — 2026-08-14(00036)에서 갱신이 멈췄고 00022~00035·00037~00039 가 빠져 있다.
--    한도의 단일 소스는 supabase/migrations/ 이며, 이 파일만으로 세운 환경은 문항 한도가
--    개편 전 값(pro = 2000)에 머문다. 아래 쿼터 함수 정의는 00021 시점 사본이라
--    현행 정책(유료 3종 모두 월 500)과 다르고, cpx_seconds 계량(00037)도 없다.
--
--    새 프로젝트를 세울 때는 이 파일을 붙여넣은 뒤 반드시
--      npx supabase db push
--    로 나머지 마이그레이션을 마저 적용할 것. 부트스트랩 편의용 스냅샷일 뿐
--    권위 있는 스키마가 아니다.
...
$$;
-- ====================== 00036_streamline_student_onboarding.sql ======================

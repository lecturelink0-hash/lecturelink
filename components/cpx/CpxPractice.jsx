...
import { Activity, Baby, Brain, CheckCircle2, ChevronDown, ChevronRight, ClipboardList, Clock3, DoorOpen, FlaskConical, MessageCircle, Mic, MicOff, MinusCircle, PersonStanding, RotateCcw, Search, Send, Shield, ShieldAlert, Sparkles, Stethoscope, UserRound, XCircle } from 'lucide-react';
...
export default function CpxPractice() {
  ...
  // 처리 안내는 현재 버전 기준 1회만 확인받는다 — 이미 확인한 사용자에게는 배너 자체를 숨긴다.
  // 'checking'(조회 중·배너 미표시) → 'needed'(배너 표시) | 'done'(확인 완료·배너 숨김)
  ...
  const [doorChart, setDoorChart] = useState(null); // 문 앞 정보 — 입실 전 인적사항·활력징후
  ...
  // 음성·대화 처리 안내는 현재 버전 기준으로 한 번만 확인받는다.
  // 서버에 확인 이력이 있으면 배너를 숨기고 체크 없이 바로 시작할 수 있다. 조회 실패 시에는 안전하게 배너를 보여준다.
  useEffect(() => {
    let active = true;
    fetch('/api/me/legal-consents')
      .then((response) => (response.ok ? response.json() : Promise.reject(new Error('consent check failed'))))
      .then((body) => {
        if (!active) return;
        if (body?.data?.cpxProcessingNotice) {
          setProcessingNoticeAccepted(true);
          setNoticeStatus('done');
        } else {
          setNoticeStatus('needed');
        }
      })
      .catch(() => { if (active) setNoticeStatus('needed'); });
    return () => { active = false; };
  }, []);
  ...
  const start = async (overrideCase, startOptions = {}) => {
    ...
    teardownRef.current = false; reconnectingRef.current = false; reconnectTriesRef.current = 0; setDoorChart(null);
    setDoorChart(null);
    ...
    try {
      ...
      setSessionId(created.sessionId); setPersona(created.persona); setDoorChart(created.doorChart || null); startedAtRef.current = Date.now(); setElapsed(0);
  ...
  const examine = async (button) => {
    ...
    try {
      ...
      setTranscript((current) => [...current, { role: 'student', text: card.transcriptText || card.declaration, tOffsetMs: Date.now() - startedAtRef.current }]);
    } catch (nextError) {
  ...
  // 검사(심전도·혈당·소변검사 등)는 손으로 보고 만지는 진찰과 다른 행위다. 한 묶음으로 두면
  // 학생이 '진찰을 했다'와 '검사를 냈다'를 구별하지 못한다(2026-08-18 감사 가3).
  // 검사는 신체 부위 필터와 무관하므로 부위 선택과 상관없이 항상 보여준다.
  const visibleButtons = buttons.filter((button) => button.kind !== 'test' && button.bodyRegion === region);
  const testButtons = buttons.filter((button) => button.kind === 'test');
  ...
  return <div className="ll-system-page cpx-page space-y-7">
    ...
    {/* 음성·대화 처리 안내 — 현재 버전 기준 미확인 사용자에게만 1회 노출 */}
    {phase === 'ready' && noticeStatus === 'needed' && (
      <section ref={noticeRef} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-sage-50)] p-4" aria-labelledby="cpx-processing-notice-title">
...
<div className="absolute inset-x-4 top-4 z-10 flex items-start justify-between gap-2">{practiceMode !== 'random' ? <div className="min-w-0 shrink rounded-[var(--radius-md)] bg-black/20 px-3 py-2 text-white"><div className="text-xs text-white/70">주소증</div><div className="truncate font-bold">{selected?.category}</div></div> : <span />}<div title={status} className="flex min-w-0 items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold text-white sm:text-sm"><Wave active={phase === 'live'} /><span className="truncate">{status}</span></div></div>{examTarget && phase === 'live' && <button type="button" onClick={() => setExamTarget(null)} className="absolute bottom-4 left-4 z-10 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3.5 py-2 text-sm font-bold text-[var(--color-primary)] shadow-lg transition hover:bg-white"><PersonStanding className="h-4 w-4" /> 환자 앉히기</button>}<div className="h-[430px]"><Avatar3D gender={persona?.gender || '여성'} age={persona?.age || 48} child={persona?.child || null} speaking={audioLevel > 0.02} audioLevel={audioLevel} pose={examTarget ? 'lying' : 'sitting'} examTarget={examTarget} category={selected?.category || ''} /></div></div><div className="border-t border-[var(--color-border)] bg-white p-4"><div className="space-y-2">{transcript.length ? transcript.slice(-3).map((event, index) => <div key={`${event.tOffsetMs}-${index}`} className={event.role === 'student' ? 'text-right' : 'text-left'}><span className={`inline-block max-w-[88%] rounded-[var(--radius-md)] px-3 py-2 text-sm ${event.role === 'student' ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-sage-100)] text-[var(--color-text)]'}`}>{event.text}</span></div>) : <p className="py-5 text-center text-sm text-[var(--color-muted)]">진료 시작 후 환자에게 질문하거나 음성으로 대화해 보세요.</p>}{transcript.length > 3 && <p className="pt-1 text-center text-[11px] text-[var(--color-muted)]">실제 시험처럼 최근 대화만 표시됩니다 · 전체 기록은 채점에 반영됩니다</p>}</div><form onSubmit={sendText} className="mt-3 flex gap-2"><input value={draft} onChange={(event) => setDraft(event.target.value)} disabled={phase !== 'live'} placeholder="보조 텍스트 입력 — 음성 문진도 자동 전사됩니다" className="h-11 min-w-0 flex-1 rounded-[var(--radius-md)] border border-[var(--color-border)] px-3 text-sm outline-none focus:border-[var(--color-primary)] disabled:bg-[var(--color-surface-muted)]"/><Button type="submit" variant="primary" disabled={phase !== 'live' || !draft.trim()}><Send className="h-4 w-4" />전송</Button></form></div></Card>
      ...
      <div className="space-y-6">{/* 문 앞 정보 — 실제 시험은 입실 전에 인적사항과 활력징후를 준다. 예전에는 활력징후가
                  진찰 버튼 뒤에 있어 쇼크 수준 혈압조차 눌러야 보였다(2026-08-18 감사 가8). */}
      {doorChart && <Card title="문 앞 정보" description="입실 전에 주어지는 정보입니다." icon={<DoorOpen className="h-5 w-5" />}><div className="space-y-1.5 text-sm"><div className="flex items-center justify-between gap-2"><span className="text-[var(--color-muted)]">인적사항</span><span className="font-bold text-[var(--color-text)]">{[doorChart.name, doorChart.age && `${doorChart.age}세`, doorChart.gender].filter(Boolean).join(" · ") || "—"}</span></div>{doorChart.child && <div className="flex items-center justify-between gap-2"><span className="text-[var(--color-muted)]">{doorChart.child.label || "환아"}</span><span className="font-bold text-[var(--color-text)]">{[doorChart.child.name, doorChart.child.age, doorChart.child.gender].filter(Boolean).join(" · ")}</span></div>}<div className="flex items-center justify-between gap-2"><span className="text-[var(--color-muted)]">주소증</span><span className="font-bold text-[var(--color-text)]">{doorChart.chiefComplaint || "—"}</span></div>{doorChart.location && <div className="flex items-center justify-between gap-2"><span className="text-[var(--color-muted)]">장소</span><span className="font-bold text-[var(--color-text)]">{doorChart.location}</span></div>}{doorChart.vitals && <div className="rounded-[var(--radius-md)] bg-[var(--color-sage-100)] p-3"><p className="text-xs text-[var(--color-muted)]">활력징후</p><p className="mt-0.5 font-bold text-[var(--color-text)]">{doorChart.vitals}</p></div>}</div></Card>}<Card title="환자 정보" icon={<UserRound className="h-5 w-5" />}><div className="space-y-2 text-sm">{persona ? (<>{persona.child && <p className="text-xs font-bold text-[var(--color-primary)]">보호자 (대화 상대)</p>}<div className="space-y-1.5">{[['이름', revealed.name ? persona.name : null], ['나이', revealed.age ? `${persona.age}세` : null], ['성별', revealed.gender ? persona.gender : null]].map(([label, val]) => <div key={label} className="flex items-center justify-between gap-2"><span className="text-[var(--color-muted)]">{label}</span><span className={val ? 'font-bold text-[var(--color-text)]' : 'text-[var(--color-muted)]'}>{val || '—'}</span></div>)}</div>{persona.child && (<><p className="pt-1 text-xs font-bold text-[var(--color-primary)]">{persona.child.label || '환아'}</p><div className="space-y-1.5">{[['이름', revealed.name ? persona.child.name : null], ['나이', revealed.age ? (persona.child.ageDetail || `${persona.child.age}세`) : null], ['성별', revealed.gender ? persona.child.gender : null]].map(([label, val]) => <div key={`child-${label}`} className="flex items-center justify-between gap-2"><span className="text-[var(--color-muted)]">{label}</span><span className={val ? 'font-bold text-[var(--color-text)]' : 'text-[var(--color-muted)]'}>{val || '—'}</span></div>)}</div></>)}{(!revealed.name || !revealed.age || !revealed.gender) && <p className="text-xs text-[var(--color-muted)]">{persona.child ? `보호자에게 ${persona.child.label || '환아'}의 이름·나이·성별을 직접 확인하세요.` : '환자에게 이름·나이·성별을 직접 여쭤보세요.'}</p>}</>) : <p className="font-bold text-[var(--color-text)]">진료 시작 시 환자 정보가 확정됩니다.</p>}{/* 증례 제목·변형에는 진단명이 들어 있다(216/243이 '급성 복통 (충수염 의심)' 꼴).
                  진료 중에 화면에 띄우면 학생은 추론할 필요 없이 답을 보고 시작한다(2026-08-18 감사 가1).
                  직접 선택이라 무엇을 골랐는지는 알지만, 진료 중에는 주소증만 남긴다. */}
              <div className="rounded-[var(--radius-md)] bg-[var(--color-sage-100)] p-3"><p className="font-bold text-[var(--color-text)]">주소증 · {selected?.category || '—'}</p><p className="mt-1 text-xs text-[var(--color-muted)]">{practiceMode === 'random' ? '무작위 실전 증례입니다. 진단과 시나리오 정보는 채점 전까지 공개되지 않습니다.' : '진단은 채점 결과에서 확인할 수 있습니다.'}</p></div></div></Card><Card title="남은 시간" icon={<Clock3 className="h-5 w-5" />} action={<span className={`tnum text-2xl font-bold ${remaining < 120 ? 'text-[var(--color-warn)]' : 'text-[var(--color-primary)]'}`}>{formatTime(remaining)}</span>}><Button fullWidth variant="accent" onClick={finish} disabled={phase !== 'live'}><Activity className="h-4 w-4" />진료 종료 및 채점</Button></Card>
      ...
      <Card title="신체진찰" description="신체 부위를 먼저 고른 뒤, 해당 부위의 진찰을 선택하세요." icon={<Stethoscope className="h-5 w-5" />}><div className="flex flex-wrap gap-2">{activeRegions.map((item) => <button key={item.id} onClick={() => { setRegion(item.id); setExamTarget(null); }} className={`rounded-full border px-3 py-1.5 text-sm font-bold transition ${region === item.id ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white' : 'border-[var(--color-border)] bg-white text-[var(--color-muted)] hover:border-[var(--color-primary)]'}`}>{item.label}</button>)}</div><div className="mt-4 grid gap-2 sm:grid-cols-2">{visibleButtons.map((button) => <Button key={button.id} variant="secondary" fullWidth disabled={phase !== 'live'} onClick={() => examine(button)}><Stethoscope className="h-4 w-4" />{button.label}</Button>)}</div>{testButtons.length > 0 && <div className="mt-5 border-t border-[var(--color-border)] pt-4"><p className="text-xs font-bold text-[var(--color-muted)]">검사 지시 — 진찰이 아니라 검사를 내는 행위입니다</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{testButtons.map((button) => <Button key={button.id} variant="secondary" fullWidth disabled={phase !== 'live'} onClick={() => examine(button)}><FlaskConical className="h-4 w-4" />{button.label}</Button>)}</div></div>}{findings.length > 0 && <div className="mt-5 grid gap-3">{findings.map((card) => <div key={card.buttonId} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-sage-50)] p-4"><div className="font-bold text-[var(--color-text)]">{card.label}</div><ul className="mt-2 space-y-1 text-sm text-[var(--color-muted)]">{card.findings.map((finding, index) => <li key={index}>• {finding.finding}</li>)}</ul></div>)}</div>}</Card>
      )}</div>
    </section>

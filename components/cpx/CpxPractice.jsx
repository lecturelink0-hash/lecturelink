...
import caseIndex from '@/data/cpx/cases.json';
import { Button } from '@/components/ui/Button';
import { Card } from '@/components/ui/Card';
import { Badge } from '@/components/ui/Badge';

const SESSION_SECONDS = 12 * 60;
const GROUPS = ['전신', '머리·목', '흉부', '팔·손목', '복부', '허리', '다리·발'];
...
function groupFor(label = '') {
  if (/눈|코|인후|목|신경|안진|안저|림프/.test(label)) return '머리·목';
  if (/심|흉|폐|호흡|청진/.test(label)) return '흉부';
  if (/맥박|혈압|상지|손|관절/.test(label)) return /관절|무릎|발|보행/.test(label) ? '다리·발' : '팔·손목';
  if (/복부|CVA|직장|소변|항문/.test(label)) return '복부';
  if (/허리|척추|SLR/.test(label)) return '허리';
  if (/발|다리|보행|하지|피부/.test(label)) return '다리·발';
  return '전신';
}
...
export default function CpxPractice() {
  const [caseId, setCaseId] = useState(caseIndex.cases[0]?.id ?? '');
  const selected = useMemo(() => caseIndex.cases.find((item) => item.id === caseId) ?? caseIndex.cases[0], [caseId]);
  ...
  const [region, setRegion] = useState('전신');
  ...
  useEffect(() => {
    ...
    request(`/exam-buttons?caseId=${encodeURIComponent(selected.id)}`)
      .then((data) => { setButtons(data.buttons || []); setRegion('전신'); })
      .catch(() => setButtons([]));
  }, [selected]);
  ...
  const visibleButtons = buttons.filter((button) => groupFor(button.label) === region);
  const activeRegions = GROUPS.filter((item) => buttons.some((button) => groupFor(button.label) === item));
  ...
  return <div className="ll-system-page space-y-7">
    ...
    <Card title="신체진찰" description="신체 부위를 먼저 고른 뒤, 해당 부위의 진찰을 선택하세요." icon={<Stethoscope className="h-5 w-5" />}><div className="flex flex-wrap gap-2">{activeRegions.map((item) => <button key={item} onClick={() => setRegion(item)} className={`rounded-full border px-3 py-1.5 text-sm font-bold transition ${region === item ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white' : 'border-[var(--color-border)] bg-white text-[var(--color-muted)] hover:border-[var(--color-primary)]'}`}>{item}</button>)}</div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{visibleButtons.map((button) => <Button key={button.id} variant="secondary" fullWidth disabled={phase !== 'live'} onClick={() => examine(button)}><Stethoscope className="h-4 w-4" />{button.label}</Button>)}</div>{findings.length > 0 && <div className="mt-5 grid gap-3">{findings.map((card) => <div key={card.buttonId} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-sage-50)] p-4"><div className="font-bold text-[var(--color-text)]">{card.label}</div><ul className="mt-2 space-y-1 text-sm text-[var(--color-muted)]">{card.findings.map((finding, index) => <li key={index}>• {finding.finding}</li>)}</ul></div>)}</div>}</Card>

...
export function MaterialImprovementStudio() {
  ...
  async function analyze() {
    if (!sourceReady || !courseId) return;
    setLoading(true);
    setError("");
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "자료를 분석하지 못했습니다.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="faculty-studio material-studio ll-upload-page">
      ...
      <header className="page-head">
        <div>
          ...
          <h1>
            내용은 그대로,
            <br />
            <span className="headline-accent">강의자료</span>를 더{" "}
            <span className="headline-accent">읽기 쉽게</span>
          </h1>
          <p className="lead">
            PPTX 또는 PDF의 내용은 보존하면서 페이지별 밀도와 정보 흐름을 분석해
            개선안을 만듭니다.
          </p>
        </div>
        <div className="guide">
          ...
          <div className="guide-panel">
            ...
            <ol>
              <li>
                <strong>자료 업로드</strong>: 개선할 PPTX 또는 PDF 파일을
                올립니다.
              </li>
              <li>
                <strong>사용 목적 선택</strong>: 정규 강의, 의료인 교육, 발표 중
                자료의 용도를 선택합니다.
              </li>
              <li>
                <strong>교수 검토</strong>: 슬라이드별 진단과 안전한 수정 제안을
                확인합니다.
              </li>
            </ol>
          </div>
      ...
      <div
        className={
          sourceReady
            ? "studio-workbench material-workbench"
            : "studio-workbench material-workbench is-upload-only"
        }
      >
        <main className="studio-main">
          ...
          >
            ...
            <div className="card-head">
              <div>
                ...
                <p>
                  가독성과 교육 흐름을 개선할 PPTX 또는 PDF 파일을 올려주세요.
                </p>
              </div>
              <div className="tag">
            ...
            {sourceReady && (
              <span className="status-copy">
                <ShieldCheck size={15} /> 원본 파일 보존 · 교수 검토 전 비공개
              </span>
            )}
            <CourseMaterialSelector
              courseId={courseId}
          ...
          {sourceReady && (
            <section
              className="studio-section material-settings card pad"
            ...
            >
              ...
              <div className="card-head">
                <div>
                  ...
                  <p>
                    기본 품질은 모두 점검하고, 자료의 사용 목적과 예외 사항만
                    반영합니다.
                  </p>
                </div>
              </div>
                  ...
                  <div className="material-request-grid">
                    <label className="field">
                      ...
                      <textarea
                        ...
                        placeholder="예: 표 안의 수치와 가이드라인 권고 문구는 그대로 유지해주세요."
                      />
                    </label>
                    <label className="field">
                      ...
                      <input
                        className="material-text-input"
                        value={lockedPages}
                        onChange={(event) => setLockedPages(event.target.value)}
                        placeholder="예: 1-3, 5, 10-12"
                      />
                    </label>
                    <label className="field">
                      ...
                      <textarea
                        ...
                        placeholder="추가로 반영할 요청을 자유롭게 입력해주세요."
                      />
                    </label>
                  </div>
                </div>
          ...
          {review && (
            <section
              className="studio-section material-results card pad"
            ...
            >
              <div className="card-head">
                ...
                <span className="material-score-badge">
              ...
              <div className="slide-review-list">
                {review.slides.map((slide) => (
                  <article key={slide.slide}>
                    <div className="slide-review-no">
                      <span>{slide.slide}</span>
                      <small>{slide.density} 밀도</small>
                    </div>
                    <div>
                      <h3>{slide.title}</h3>
                      ...
                      <p className="slide-recommendation">
                        <b>개선안</b>
                        {slide.recommendation}
                      </p>
                      <div className="safe-actions">
                        {slide.safeActions.map((action) => (
            ...
            steps={[
              {
                ...
                title: "사용 목적과 요청 확인",
                description:
                  "자료의 사용 목적과 반드시 유지할 내용을 선택적으로 입력합니다.",
              },
              {
                number: 3,
                title: "페이지 구조 분석",
                description: "텍스트 밀도와 제목 위계, 정보 흐름을 진단합니다.",
              },
              {
                number: 4,
                title: "개선안 검토·저장",
                description:
                  "슬라이드별 제안을 확인하고 선택한 차시에 저장합니다.",
              },
            ]}
            footer={
              <>
                먼저 왼쪽 <b className="text-sage-700">1. 자료 업로드</b>에서
                PPTX 또는 PDF 파일을 선택해주세요.
              </>
            }
          />
        )}
          ...
          <aside className="faculty-summary summary summary-hero card pad">
            ...
            <dl className="summary-list">
              ...
              <div className="summary-item">
                <span>사용 목적</span>
                <strong>
                  {purpose === "기타" ? customPurpose || "직접 입력" : purpose}
                </strong>
              </div>
              <div className="summary-item">
                <span>기본 점검</span>
                <strong>6개 기준 전체</strong>
              </div>
              <div className="summary-item">
                <span>추가 요청</span>
                <strong>
                  {mustKeep || lockedPages || additionalPrompt
                    ? "입력됨"
                    : "없음"}
                </strong>
              </div>
            ...
            <button
              ...
              disabled={
                ...
                !courseId ||
                loading ||
                (purpose === "기타" && !customPurpose.trim())
              }
              onClick={analyze}
            >
              {loading ? (
                <>
                  <Loader2 className="spin" size={17} /> 자료 분석 중
                </>
              ) : (
                <>
                  개선안 만들기 <ArrowRight size={17} />
                </>
              )}
            </button>
            {review?.artifactId && (
            ...
            <p className="summary-note note">
              <FileText size={14} /> 결과는 선택한 차시에 저장되며 원본 파일을
              직접 변경하지 않습니다.
            </p>
          </aside>
        )}

...
export function PrerequisiteBridgeStudio() {
  ...
  useEffect(() => { fetch('/api/professor/courses').then(response => response.json()).then(payload => payload.ok && setCourses(payload.data)); }, []);

  async function generate() {
    if (!file || !courseTopic.trim() || !courseId) return;
    setLoading(true); setError(''); setResult(null);
    const form = new FormData();
    form.append('file', file); form.append('courseId', courseId); form.append('courseTopic', courseTopic); form.append('learnerLevel', learnerLevel); form.append('reviewLength', reviewLength); form.append('emphasis', emphasis);
    try {
  ...
  return <div className="bridge-studio">
    ...
    </main><aside className="bridge-run"><BookOpenCheck size={20}/><h2>예습자료 초안</h2><dl><div><dt>저장할 차시</dt><dd><select value={courseId} onChange={event=>setCourseId(event.target.value)}><option value="">차시 선택</option>{courses.map(course=><option key={course.id} value={course.id}>{course.title}</option>)}</select></dd></div><div><dt>주제</dt><dd>{courseTopic||'입력 전'}</dd></div><div><dt>학습자</dt><dd>{learnerLevel}</dd></div><div><dt>분량</dt><dd>{reviewLength}</dd></div><div><dt>자료</dt><dd>{file?.name??'선택 전'}</dd></div></dl><button type="button" disabled={!file||!courseTopic.trim()||!courseId||loading} onClick={generate}>{loading?<><Loader2 className="bridge-spin" size={17}/> 초안 생성 중</>:<>만들고 차시에 저장</>}</button>{result?.artifactId&&<Link className="workspace-return" href={`/professor/courses/${courseId}`}>저장됨 · 차시로 돌아가기</Link>}<p>결과는 선택한 차시에 자동 저장되며 학생 공개 전 교수 검토가 필요합니다.</p></aside></div>
  </div>;
}

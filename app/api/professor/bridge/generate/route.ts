...
async function generateMedicalArtwork(result: z.infer<typeof resultSchema>) {
  ...
  const readinessPrompt = result.readinessCheck.length === 2
...
Tiny answer line: 정답 1) ${result.readinessCheck[0].answer}  2) ${result.readinessCheck[1].answer}`
    : 'Do not add prerequisite questions or an answer section.';
...
${result.readinessCheck.length === 2
  ? '- Put the two questions at the bottom; answers must be faint and small.'
  : '- Do not reserve space for questions or answers.'}
...
function createOutputTool(includeReadiness: boolean) {
  return {
  ...
  input_schema: {
    ...
    properties: {
      ...
      readinessCheck: { type: 'array', minItems: includeReadiness ? 2 : 0, maxItems: includeReadiness ? 2 : 0, items: { type: 'object', required: ['question', 'answer'], properties: { question: { type: 'string' }, answer: { type: 'string' } } } },
...
- 디자인이 auto이면 주제의 정보 구조에 맞춰 스타일을 선택한다: 기전/경로는 blueprint, 해부/임상 흐름은 medical-clean, 기억법은 hand-drawn, 비교/개요는 editorial.`,
    tools: [createOutputTool(settings.includeReadiness)],
  ...
  if (result.readinessCheck.length !== (settings.includeReadiness ? 2 : 0)) {
    throw new ApiException('generation_invalid_output', '선수지식 확인 문항 설정을 결과에 반영하지 못했습니다. 다시 생성해주세요.', 502);
  }

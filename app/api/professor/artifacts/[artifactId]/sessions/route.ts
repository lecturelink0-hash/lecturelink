...
export const POST=withErrorHandling(async(_r:Request,c:{params:Promise<{artifactId:string}>})=>{
  ...
  const {data:a}=await db.from('learning_artifacts').select('id,course_id,title,formative_items(id,position,stem,choices,answer_index,explanation,objective,source_pages,image_data_url,approved)').eq('id',artifactId).eq('created_by',user.userId).single();

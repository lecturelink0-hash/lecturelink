...
const question = z.object({ stem:z.string().min(1), choices:z.array(z.string()).min(2), answerIndex:z.number().int().min(0), explanation:z.string().default(''), objective:z.string().default(''), sourcePages:z.array(z.number()).default([]), cognitiveLevel:z.string().optional(), qualityFlags:z.array(z.string()).default([]), imageDataUrl:z.string().startsWith('data:image/').nullable().default(null) });
const schema = z.object({ title:z.string().min(1), sourceName:z.string().optional(), materialId:z.string().uuid().optional(), summary:z.string().optional(), objectives:z.array(z.string()).default([]), questions:z.array(question).min(1) });

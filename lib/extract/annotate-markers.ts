...
/**
 * 발문이 표식 글자를 가리키는지 — **어느 판을 줄지 고를 때만 쓰는 넓은 판정.**
 *
 * 위 좁은 규칙은 오탐 대가가 문항 삭제라 일부러 좁다. 그런데 같은 함수를 "표식판을
 * 줄 것인가"에도 쓰고 있었고, 그 자리에서는 대가가 정반대다.
 *
 *   미탐 → 표식 없는 판이 배정된다 → 학생이 **풀 수 없는 문항**이 나간다.
 *   오탐 → 표식이 있는 그림에 표식판이 간다 → 안 물어본 표식이 보인다(거슬리는 정도).
 *
 * 운영 실측(2026-08-19): "이 그림의 **D 단계**에서 얻을 수 있는 정보와 관련된 뇌파는?"
 * 이 좁은 규칙에 안 걸려 표식 없는 판을 받았다. 그날 표식 문항 2건 중 1건이 그랬다.
 * 좁게 둔 판단 자체는 옳았고, 틀린 것은 **대가가 반대인 두 자리에 같은 규칙을 쓴 것**이다.
 *
 * 넓히되 홑글자를 아무 데서나 받지는 않는다. "B형 간염"·"비타민 D 결핍"·"B 세포"처럼
 * 글자가 분류명·물질명으로 쓰이는 경우와 가르는 단서는 **뒤에 오는 명사**뿐이므로,
 * 도형을 가리킬 때만 쓰는 명사를 닫힌 목록으로 두고 그때만 참으로 본다. 목록에 없는
 * 명사가 붙으면 종전대로 거짓이다 — 넓힌 쪽도 여전히 열거된 형태만 받는다.
 */
const MARKER_POINTER_NOUNS =
  '단계|단면|부위|부분|구조(?:물)?|영역|구역|지점|위치|층|칸|상자|화살표|번호|기호';

const MARKER_REFERENCE_LOOSE_RE = new RegExp(
  // ① 좁은 규칙(표시·가리키·지시)을 그대로 품는다.
  `${MARKER_REFERENCE_RE.source}` +
    // ② 홑글자 + 도형 지시 명사. 앞뒤에 다른 영문이 붙으면 단어의 일부다(Astrocyte).
    `|(?<![A-Za-z])[A-E](?![A-Za-z])\\s*(?:의|에|에서|와|과|은|는|이|가)?\\s*(?:${MARKER_POINTER_NOUNS})` +
    // ③ 동그라미 글자·괄호 글자는 그 자체가 표식 표기다(모델이 이렇게 쓰기도 한다).
    `|[\\u24B6-\\u24BA]|\\(\\s*[A-E]\\s*\\)`,
);

export function stemReferencesMarkerLoose(stem: string): boolean {
  return MARKER_REFERENCE_LOOSE_RE.test(String(stem ?? ''));
}

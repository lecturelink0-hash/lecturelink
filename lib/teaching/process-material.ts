...
export async function processTeachingMaterial(materialId: string, professorId: string) {
  ...
  try {
    ...
    const file = new File([await blob.arrayBuffer()], material.file_name, { type: material.mime_type });
    ...
    console.error('[teaching-material]', { stage: 'processing_failed', materialId, fileType: material.file_type, errorCode: classified.code });

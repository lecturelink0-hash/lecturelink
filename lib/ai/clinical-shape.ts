...
const CLINICAL_DETAIL_PATTERNS: RegExp[] = [
  // 활력징후. **조사를 허용해야 한다** — 2026-08-18 운영 실측에서 "혈압은 170/100 mmHg로
  // 측정되었고"가 통과하지 못해, 명백한 증례가 지식형으로 세어졌다.
  /(?:혈압|맥박|체온|호흡|산소포화도)\s*(?:은|는|이|가)?\s*\d/,
  // 진찰. **"진찰 시"를 허용해야 한다** — 같은 실측에서 "진찰 시 양측 요골 동맥 맥박이
  // 촉지되지 않았다"가 걸리지 않았다. 국시 서식은 "진찰에서"지만 강의자료 기반 생성은
  // 그 서식을 따르지 않는다(따를 이유도 없다 — 여기는 세는 자리지 교정하는 자리가 아니다).
  /(?:진찰|청진|촉진|시진|타진)\s*(?:에서|상|시)/,
  // 검사·영상. 검사명이 앞에 붙는 형태("흉부 CT 결과", "초음파 소견")까지 센다.
  /검사\s*결과|검사에서|영상에서|사진에서|소견(?:이|은|을|에서)/,
  /(?:CT|MRI|X선|초음파|내시경|심전도|조영술|영상|촬영)\s*(?:결과|소견|에서|상)/i,
  /(?:하|되|있|없|보이|들리)(?:였|었)다고?\s*한다/, // 전언
  // 검사·진찰 소견. 과거 시제도 센다("관찰되었다"). 앞의 "에서"가 범위를 묶어 준다.
  /에서\s*[^.?!\n]{0,40}?(?:보인다|보였다|관찰된다|관찰되었다|들린다|들렸다|만져진다|만져졌다|나타난다|나타났다|측정된다|측정되었다)/,

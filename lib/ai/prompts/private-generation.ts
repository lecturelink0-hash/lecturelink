...
- 특정 이미지를 판독·기반으로 하는 문항은 \`image_indices\` 에 해당 이미지 번호들(0-based)을 넣는다. 한 문항이 여러 이미지를 비교·종합하면 여러 번호를 넣는다 (예: EKG + 흉부 X-ray 비교 → [0, 1]).
- **억지 연결 금지**: \`image_indices\` 는 "이미지를 직접 보지 않으면 풀 수 없는" 판독 문항에만 채운다. 텍스트만으로 풀리는 문항은 반드시 빈 배열 [] 로 둔다.

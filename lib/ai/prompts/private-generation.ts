...
- **문두(질문)는 짧은 명사구를 우선한다.** 예: "진단은?", "원인은?", "기전은?", "기준은?", "확진 검사는?", "치료는?".
  - 사실을 나열해 고르게 할 때만 "옳은 것은?", "옳지 않은 것은?" 형태를 쓴다.
  - **금지: "가장 적절한", "가장 가능성 높은", "다음 중".** ("가장 흔한 원인은?"처럼 사실 자체가
    최빈값인 경우와 "가장 먼저 시행할 검사는?"처럼 순서를 묻는 경우만 예외다.)
    실측 근거: 국시 공개 기출 79문항에 "가장 적절한"은 0건이다. 이 표현은 채점 기준을 흐리고
    선지 사이의 우열을 모호하게 만든다.
...
- **해설의 구조를 고정한다.** 이 순서·분량 그대로 쓴다.
  1. 정답이 옳은 임상적 근거 **2문장**.
  2. 이어서 **오답 4개를 각각 한 문장씩**, 번호로 지목해서 쓴다. 예: "②는 발열이 없어 아니다." "④는 침범 부위가 달라 아니다."
     - 오답 4개를 **하나도 빠뜨리지 않는다.** 정답만 설명하고 끝내지 않는다.
- **간결성(길이 상한)**: 해설 전체는 **350자 이내**다. 교과서식 배경 설명·같은 내용 반복·불필요한 서론은 넣지 않는다. 위 구조(2문장 + 4문장)를 지키면 자연히 상한 안에 들어온다.
- **문체는 평서체(~이다 / ~한다)로 통일한다.** "~입니다", "~합니다", "~있습니다" 같은 존댓말을 쓰지 않는다. 실제 국시 해설이 평서체이고, 한 세트 안에서 문체가 섞이면 학생이 그것부터 본다.
        ...
        items: {
          ...
          properties: {
            stem: {
              type: 'string',
              description:
                '지문 + 문두. 문두는 짧은 명사구를 우선한다(예: "진단은?", "원인은?", "기준은?"). 사실을 고르게 할 때만 "옳은 것은?" / "옳지 않은 것은?" 을 쓴다. **"가장 적절한", "가장 가능성 높은", "다음 중" 은 쓰지 않는다**("가장 흔한 ~은?", "가장 먼저 시행할 ~은?" 만 예외). "무엇인가요?", "인가요?", "까요?" 같은 구어체 종결은 절대 쓰지 않는다. "그림 1.", "이미지 1:", "(흉부 X선 사진)" 같은 이미지 캡션·그림 설명 문구를 넣지 않는다.',
            },
            choices: {
              type: 'array',
              items: { type: 'string' },
            ...
            explanation: {
              type: 'string',
              description:
                '정답의 임상적 근거와 각 오답이 틀린 이유를 담은, 그 자체로 완결된 임상 해설. **구조 고정**: 정답 근거 2문장 + 오답 4개를 번호로 지목해 각각 한 문장("②는 … 아니다"), 오답을 하나도 빠뜨리지 않는다. 전체 350자 이내. **평서체(~이다/~한다)로만 쓰고 "~입니다"·"~합니다" 같은 존댓말을 쓰지 않는다.** "강의 자료에 따르면"·"업로드된 자료에서" 등 자료 출처 언급을 절대 하지 말 것.',
            },
            concepts: {
              type: 'array',
              items: { type: 'string' },
            ...
            difficulty: {
              ...
              maximum: 3,
              description:
                '이 문항이 실제로 요구하는 인지 수준 = solution_conditions 의 서로 다른 항목 수로 정한다. 1=재인(조건·지식 1개로 풀린다), 2=적용(서로 다른 조건·지식 2개를 결합해야 풀린다), 3=분석(서로 다른 조건·지식 3개 이상을 결합해야 풀린다). **요청 난이도에 맞추려 값을 올리거나 내리지 말고, 만들어진 문항의 실제 수준을 정직하게 적는다** — 서버가 solution_conditions 의 수와 대조한다.',
            },
            solution_conditions: {
              type: 'array',
              items: { type: 'string' },
              description:
                '정답을 확정하기 위해 **반드시 결합해야 하는 서로 다른 조건·지식**을 각각 한 구절로 적는다. 지문에 제시된 조건(증상·수치·병력·그림 소견)은 지문의 표현 그대로, 풀이에 필요한 지식은 "…이면 …이다" 꼴의 한 문장으로. 그중 하나라도 빼면 정답이 다른 선지와 갈리지 않아야 진짜 조건이다. 같은 조건을 말만 바꿔 두 번 적지 않는다. 사실 하나만 알면 풀리는 문항은 정직하게 1개만 적는다 — 서버가 이 목록의 고유 항목 수로 난이도를 판정한다.',
            },
            ask_kind: {
              type: 'string',
              description:
                '이 문항이 묻는 것의 유형. 지식형: definition(정의·특징) / mechanism(기전) / cause_risk(원인·위험인자) / classification(분류·비교) / number_criteria(수치·기준) / test_interpretation(검사·해석) / complication_prognosis(합병증·예후). 임상형: diagnosis(진단) / next_test(다음 검사) / initial_management(초기 처치) / treatment(치료) / drug(약물) / cause_organism(원인 병체) / prevention(예방). 이미지형: image_finding(영상 소견) / image_diagnosis(영상 진단) / image_structure(구조 지목). 한 묶음 안에서 같은 값을 반복하지 않는다.',
            },
            image_indices: {
              type: 'array',
              items: { type: 'integer' },
          ...
          required: [
            ...
            'difficulty',
            'ask_kind',
            // 난이도의 근거 목록. 선택 필드로 두면 모델이 조용히 빼먹어 서버가 셀 것이 없어진다
            // (source_pages 와 같은 이유). 빈 배열은 허용하되 필드는 반드시 채우게 한다.
            'solution_conditions',
            'image_indices',
            'sub_topic_code',

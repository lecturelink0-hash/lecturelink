...
- 사실 하나만 알면 풀리는 문항("…의 금기 약물은?", "…의 기전은?", "…과 관련된 것은?",
  "…에 사용하는 검사는?")은 **1개**입니다. 발문이 길거나 수식어가 많아도 정답을 가르는 정보가 하나면 1개입니다.
- **정답을 양성으로 확정하는 데 필요한 것만** 셉니다. "각 오답 선지가 왜 틀렸는지 아는 지식"은
  세지 않습니다 — 5지선다는 어느 문항이든 오답 4개가 있으므로, 그것을 세면 모든 문항이 4개 이상이 됩니다.
- "…에 대한 설명으로 옳은 것은?", "…의 특징으로 옳은 것은?"처럼 선지 하나하나가 독립된 사실이고
  정답 선지의 사실 하나만 알면 고를 수 있는 문항은 **1개**입니다.
- 3개 이상은 **발문이 제시한 서로 다른 조건(증상·수치·병력·분류·그림 소견)** 을 실제로 결합해야
  정답이 좁혀질 때만입니다. 조건이 발문에 적혀 있어도 그것 없이 정답이 정해지면 세지 않습니다.

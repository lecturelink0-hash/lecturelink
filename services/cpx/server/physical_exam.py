...
HAND_TREMOR_BUTTONS = [
    ...
    # 두 버튼의 키워드는 서로 겹치지 않아야 한다. '떨림' 하나로 매칭하면 안정떨림 버튼이
    # 자세·동작 항목까지 함께 집어 오고, 정작 자세·동작 버튼은 케이스 항목('자세·동작 떨림')과
    # 표기가 달라 매칭에 실패해 기본값(본태떨림 소견)을 내보냈다 — 파킨슨 케이스에서 한 환자에
    # 상반된 두 소견이 나오던 원인(2026-08-18 감사).
    {'id': 'rest_tremor', 'label': '안정떨림 관찰', 'keywords': ['안정떨림', '안정 시 떨림'],
     'declaration': '손을 무릎이나 탁자 위에 편하게 올려두신 상태에서 떨림을 관찰하겠습니다.', 'defaultFinding': '안정 시 뚜렷한 떨림 없음.'},
    {'id': 'postural_action_tremor', 'label': '자세·동작 떨림/글씨 검사', 'keywords': ['자세떨림', '동작떨림', '자세·동작', '나선', '글씨'],
     'declaration': '팔을 앞으로 뻗고 손바닥을 편 다음, 나선을 그리고 글씨를 써 보시며 떨림을 확인하겠습니다.', 'defaultFinding': '자세·동작 시 뚜렷한 떨림은 관찰되지 않음.'},
...
BREAST_PAIN_MASS_BUTTONS = [
    ...
    {'id': 'breast_palp', 'label': '유방 촉진 (설명·동의)', 'keywords': ['유방 촉진', '유방 멍울', '유방 종괴', '유방 압통'],
     'declaration': '누운 자세에서 유방을 체계적으로 만져 덩이와 압통을 확인하겠습니다. 괜찮으시면 시작하겠습니다.', 'defaultFinding': '유방에서 뚜렷한 덩이나 압통은 확인되지 않습니다.'},
...
VAGINAL_DISCHARGE_BUTTONS = [
    ...
    {'id': 'speculum', 'label': '질경 진찰/검체 채취 (설명·동의)', 'keywords': ['질경', '질분비물', '자궁경부', '검체'],
     'declaration': '질과 자궁경부, 분비물을 확인하고 필요한 검체를 채취하기 위해 질경 진찰을 하겠습니다. 언제든 중단하실 수 있습니다.', 'defaultFinding': '동의 후 시행 시 질·자궁경부에 뚜렷한 병변이나 이상 분비물은 보이지 않습니다.'},
...
DEVELOPMENTAL_DELAY_BUTTONS = [
    ...
    {'id': 'development', 'label': '놀이·언어·사회성 관찰', 'keywords': ['발달 관찰', '운동 발달', '언어 발달', '사회성', '놀이'],
     'declaration': '아이와 놀이를 하며 눈맞춤·이해·표현 언어와 사회적 상호작용을 관찰하겠습니다.', 'defaultFinding': '연령에서 기대되는 놀이·언어·사회적 상호작용을 대체로 보입니다.'},
...
SUBSTANCE_MISUSE_BUTTONS = [
    ...
    {'id': 'pupil_neuro', 'label': '동공·신경·떨림·보행', 'keywords': ['동공', '신경', '근력', '반사', '떨림', '보행'],
     'declaration': '동공과 눈 움직임, 팔다리 힘과 반사, 떨림과 걸음걸이를 확인하겠습니다.', 'defaultFinding': '동공과 신경학적 진찰에서 특이소견이 없습니다.'},
...
DOMESTIC_VIOLENCE_BUTTONS = [
    ...
    {'id': 'skin_injury', 'label': '피부·사지 손상 (설명·동의)', 'keywords': ['피부', '사지', '손상', '멍'],
     'declaration': '환자분이 허용하시는 범위에서 피부와 팔다리의 상처를 확인하겠습니다. 언제든 중단하실 수 있습니다.', 'defaultFinding': '환자분이 허용한 범위에서 확인한 피부와 팔다리에 뚜렷한 손상은 보이지 않습니다.'},
...
SEXUAL_VIOLENCE_BUTTONS = [
    ...
    {'id': 'skin_injury', 'label': '전신·피부 손상 (설명·동의)', 'keywords': ['전신', '피부', '손상', '멍'],
     'declaration': '환자분이 허용하시는 범위에서 노출을 최소화하며 전신의 상처를 확인하겠습니다.', 'defaultFinding': '환자분이 허용한 범위에서 확인한 전신 피부에 뚜렷한 손상은 보이지 않습니다.'},
    ...
    {'id': 'external_genital', 'label': '외음부 진찰 (별도 동의)', 'keywords': ['외음부', '생식기'],
     'declaration': '외음부 진찰은 별도로 설명드린 뒤 동의하실 때만 시행하며 언제든 중단할 수 있습니다.', 'defaultFinding': '별도 동의 후 확인 시 외음부에 뚜렷한 손상은 보이지 않습니다.'},
...
SMOKING_BUTTONS = [
    ...
    {'id': 'oral_finger', 'label': '구강·손가락 확인', 'keywords': ['구강', '치아', '착색', '손가락', '변색'],
     'declaration': '입안 치아 착색과 손가락 변색 등 흡연 흔적을 확인하겠습니다.', 'defaultFinding': '치아 착색과 손가락 변색은 뚜렷하지 않습니다.'},
    ...
    {'id': 'co_test', 'label': '호기 일산화탄소 측정', 'keywords': ['일산화탄소', 'CO', '코티닌'],
     'declaration': '내쉬는 숨의 일산화탄소 농도를 측정하겠습니다.', 'defaultFinding': '호기 일산화탄소는 비흡연자 범위입니다.'},
]
